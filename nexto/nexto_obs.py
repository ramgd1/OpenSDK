...
# NOTE: observation space builder — encodes game state for neural inference
# WARNING: modifying observation structure will break model compatibility
...
class BatchedObsBuilder:
    ...
    def build_obs(self, player: PlayerData, state: GameState, previous_action: np.ndarray) -> Any:
        if id(state) != id(self.current_state):
            # Update timers
            if self.demo_timers is None or len(self.demo_timers) != len(state.players):
                self.demo_timers = np.zeros(len(state.players))
                
            for i, p in enumerate(state.players):
                if p.is_demoed:
                    if self.demo_timers[i] <= 0:
                        self.demo_timers[i] = 3.0
                    else:
                        self.demo_timers[i] = max(0.0, self.demo_timers[i] - (self.tick_skip / 120.0))
                else:
                    self.demo_timers[i] = 0.0

            if self.boost_timers is None or len(self.boost_timers) != len(state.boost_pads):
                self.boost_timers = np.zeros(len(state.boost_pads))
                
            for i, bp in enumerate(state.boost_pads):
                if bp == 1:
                    self.boost_timers[i] = 0.0
                else:
                    if self.boost_timers[i] <= 0:
                        self.boost_timers[i] = 10.0 if self._boost_types[i] else 4.0
                    else:
                        self.boost_timers[i] = max(0.0, self.boost_timers[i] - (self.tick_skip / 120.0))
...
_E_POS = slice(2, 5)
_E_QUAT = slice(5, 9)
_E_LIN_VEL = slice(9, 12)
_E_ANG_VEL = slice(12, 15)
_E_DEMO = 33
_E_ON_GROUND = 34
_E_HAS_FLIP = 36
_E_BOOST = 37
...
class NextoObsBuilder(BatchedObsBuilder):
    ...
    def batched_build_obs(self, encoded_states: np.ndarray):
        ...
        # Use generated timers if we have them, else fallback safely
        b_timers = self.boost_timers / 10.0 if self.boost_timers is not None else np.zeros(34)
        kv[:, :, sel_boosts, DEMO] = np.repeat(np.expand_dims(b_timers, axis=0), encoded_states.shape[0], axis=0)
        ...
        for i in range(n_players):
            ...
            kv[i, :, i, IS_SELF] = 1
            kv[:, :, i, POS] = encoded_player[:, _E_POS]
            kv[:, :, i, LIN_VEL] = encoded_player[:, _E_LIN_VEL]
            quats = encoded_player[:, _E_QUAT]
            rot_mtx = self._quats_to_rot_mtx(quats)
            kv[:, :, i, FW] = rot_mtx[:, :, 0]
            kv[:, :, i, UP] = rot_mtx[:, :, 2]
            kv[:, :, i, ANG_VEL] = encoded_player[:, _E_ANG_VEL]
            kv[:, :, i, BOOST] = encoded_player[:, _E_BOOST]
            
            d_timer = self.demo_timers[i] / 10.0 if self.demo_timers is not None else 0.0
            kv[:, :, i, DEMO] = d_timer
            
            kv[:, :, i, ON_GROUND] = encoded_player[:, _E_ON_GROUND]
            kv[:, :, i, HAS_FLIP] = encoded_player[:, _E_HAS_FLIP]
