# WARNING: game object definitions — maps raw memory offsets to structured data
# Modifying offsets will corrupt the input pipeline
import ctypes
    ...
    def is_a(self, class_name):
        ...
        while current_class is not None:
            if current_class.get_full_name() == class_name:
                return True
            # Make sure get_super_field returns the correct parent, not a generic UField
...
class UField(UObject):
    ...
    def get_next(self):
        # Read the address of the next field ('Next') from memory
        next_field_addr = self.sdk.pm.read_ulonglong(self.address + 0x0060)
        if next_field_addr != 0:  # Check if the address is non-zero
            return UField(next_field_addr, sdk=self.sdk)  # Or an appropriate subclass if needed
        return None
...
class UClass(UObject):
    ...
    def get_super_field(self):
        # Read the SuperField address from memory
        super_field_addr = self.sdk.pm.read_ulonglong(self.address + 0x0080)  # Replace 'offset_SuperField' with the actual offset
        if super_field_addr != 0:  # Check if the address is non-zero
            return UField(super_field_addr, sdk=self.sdk)  # Or an appropriate subclass if needed
        return None

class GameEvent(UObject):
    def __init__(self, address = None, sdk=None):
        super().__init__(address, sdk=sdk)
        self.balls_offset = 0x08E8
        self.cars_offset = 0x0350
        self.pris_offset = 0x0340
        self.teams_offset = 0x0758
        self.players_offset = 0x0330
        self.local_players_offset = 0x0360

    def get_balls(self):
        balls_tarray_address = self.address + 0x08E8
        return TArray(balls_tarray_address, Ball, sdk=self.sdk).get_items()

    def get_teams(self):
        teams_tarray_address = self.address + 0x0758
        return TArray(teams_tarray_address, TeamInfo, sdk=self.sdk).get_items()

    def get_pris(self):
        pris_tarray_address = self.address + 0x0340
        return TArray(pris_tarray_address, PRI, sdk=self.sdk).get_items()

    def get_cars(self):
        cars_tarray_address = self.address + 0x0350
        return TArray(cars_tarray_address, Car, sdk=self.sdk).get_items()
    ...
    def get_game_time(self):
        return self.sdk.pm.read_int(self.address + 0x083C)
    
    def get_warmup_time(self):
        return self.sdk.pm.read_int(self.address + 0x0840)
    
    def get_max_score(self):
        return self.sdk.pm.read_int(self.address + 0x0844)
    
    def get_time_remaining(self):
        return self.sdk.pm.read_float(self.address + 0x0874)
        
    def get_seconds_remaining(self):
        return self.sdk.pm.read_int(self.address + 0x0878)
    
    def get_total_game_time_played(self):
        return self.sdk.pm.read_float(self.address + 0x0880)
    
    def get_overtime_played(self):
        return self.sdk.pm.read_float(self.address + 0x0884)

    def is_round_active(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 2) & 1
    
    def is_play_replays(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 3) & 1
    
    def is_ball_has_been_hit(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 4) & 1
    
    def is_overtime(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 5) & 1
    
    def is_unlimited_time(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 6) & 1
    
    def is_no_contest(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 8) & 1
    
    def is_disable_goal_delay(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 10) & 1
    
    def is_show_no_scorer_goal_message(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 11) & 1
    
    def is_match_ended(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 13) & 1
    
    def is_show_intro_scene(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 16) & 1
    
    def is_club_match(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 18) & 1
    
    def is_can_drop_online_rewards(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 21) & 1
    
    def is_allow_honor_duels(self):
        return (self.sdk.pm.read_int(self.address + 0x0808) >> 22) & 1
    
    def get_match_winner(self):
        team_address = self.sdk.pm.read_ulonglong(self.address + 0x0940)
        return Team(team_address, sdk=self.sdk)
    
    def get_game_winner(self):
        team_address = self.sdk.pm.read_ulonglong(self.address + 0x0938)
        return Team(team_address, sdk=self.sdk)
    
    def get_mvp(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0958)
        return PRI(pri_address, sdk=self.sdk)
    
    def get_fastest_goal_player(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0960)
        return PRI(pri_address, sdk=self.sdk)
    
    def get_slowest_goal_player(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0968)
        return PRI(pri_address, sdk=self.sdk)
    
    def get_furthest_goal_player(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0970)
        return PRI(pri_address, sdk=self.sdk)
    
    def get_fastest_goal_speed(self):
        return self.sdk.pm.read_float(self.address + 0x0978)
    
    def get_slowest_goal_speed(self):
        return self.sdk.pm.read_float(self.address + 0x097C)
    
    def get_furthest_goal(self):
        return self.sdk.pm.read_float(self.address + 0x0980)
    ...
    def get_round_num(self):
        return self.sdk.pm.read_int(self.address + 0x0990)
    ...
    def get_goals(self):
        goals_tarray_address = self.address + 0x0908
...
class Ball(Actor):
    ...
    def get_location(self) -> FVector:
        # RBState (0x05D0) + Location (0x10)
        return FVector(self.address + 0x05E0, sdk=self.sdk)

    def get_velocity(self) -> FVector:
        # RBState (0x05D0) + LinearVelocity (0x1C)
        return FVector(self.address + 0x05EC, sdk=self.sdk)
    
    def get_rotation(self) -> FRotator:
        # Inherit Actor.Rotation (0x9C) which is usually synced for non-sleeping actors
        return FRotator(self.address + 0x009C, sdk=self.sdk)

class FString(Pointer):
    def get_string(self):
        # Read the character array address
        ...
        for i in range(array_count - 1):  # -1 because there is often a null terminator
            char = self.sdk.pm.read_ushort(array_data_address + (i * 2))  # read 2 bytes at a time
            result += chr(char)
...
class PlayerReplicationInfo(Actor):
    ...
    def get_player_name(self):
        return FString(self.address + 0x0288, sdk=self.sdk).get_string()
    
    def get_team_info(self):
        team_address = self.sdk.pm.read_ulonglong(self.address + 0x02B0)
        return TeamInfo(team_address, sdk=self.sdk)
    ...
    def is_admin(self):
        return (self.sdk.pm.read_int(self.address + 0x02B8) & 1) != 0
...
class BoostComponent(Pointer):
    ...
    def get_amount(self):
        return self.sdk.pm.read_float(self.address + 0x0338)
    
    def get_max_amount(self):
        return self.sdk.pm.read_float(self.address + 0x032C)
    
    def get_consumption_rate(self):
        return self.sdk.pm.read_float(self.address + 0x0328)
    
    def get_start_amount(self):
        return self.sdk.pm.read_float(self.address + 0x0330)

class Vehicle(Pawn):
    size = 0x08A8
    
    def get_pri(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0810)
        return PRI(pri_address, sdk=self.sdk)   

    def get_inputs(self):
        input_address = self.address + 0x07DC
        return VehicleInputs(input_address, sdk=self.sdk)
    
    def get_boost_component(self):
        boost_component_address = self.sdk.pm.read_ulonglong(self.address + 0x0850)
        return BoostComponent(boost_component_address, sdk=self.sdk)

    def get_vehicle_sim(self):
        vehicle_sim_address = self.sdk.pm.read_ulonglong(self.address + 0x07C0)
    ...
    def is_driving(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) & 1) != 0
    
    def is_on_ground(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 4) & 1
    
    def is_supersonic(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 5) & 1
    
    def is_jumped(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 2) & 1

    def is_double_jumped(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 3) & 1
    
    def is_podium_mode(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 6) & 1
    
    def has_post_match_celebration(self):
        return (self.sdk.pm.read_int(self.address + 0x07D8) >> 7) & 1
...
class Car(Vehicle):
    ...
    def get_game_event(self):
        game_event_address = self.sdk.pm.read_ulonglong(self.address + 0x0A98)
        return GameEvent(game_event_address, sdk=self.sdk)

    def get_attacker_pri(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x0A48)
        return PRI(pri_address, sdk=self.sdk)
...
class PlayerController(Controller):
    ...
    def __init__(self, address, *, sdk=None):
        ...
        self.chat_message = self.sdk.create_callable_function(
            ...
            arg_types={
                "InPRI": "pointer",
                "Message": "fstring",  # Size will be adjusted dynamically
                "ChatChannel": "uint32",
                "bPreset": "uint32"
    ...
    def get_pri(self):
        pri_address = self.sdk.pm.read_ulonglong(self.address + 0x09A0)
        return PRI(pri_address, sdk=self.sdk)

    def get_car(self):
        car_address = self.sdk.pm.read_ulonglong(self.address + 0x0998)
        return Car(car_address, sdk=self.sdk)
...
class VehiclePickup(Actor):
    ...
    def get_respawn_delay(self):
        return self.sdk.pm.read_float(self.address + 0x026C)
...
class Goal(UObject):
    ...
    def get_location(self):
        # UGoal_TA.Location at 0x0138
        return FVector(self.address + 0x0138, sdk=self.sdk)

    def get_direction(self):
        # UGoal_TA.Direction at 0x0144
        return FVector(self.address + 0x0144, sdk=self.sdk)

    def get_rotation(self):
        # UGoal_TA.Rotation at 0x0168
        return FRotator(self.address + 0x0168, sdk=self.sdk)
...
class GameViewportClient(UObject):
    ...
    def get_game_event(self):
        game_event_address = self.sdk.pm.read_ulonglong(self.address + 0x0300)
        return GameEvent(game_event_address, sdk=self.sdk)
